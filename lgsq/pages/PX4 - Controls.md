...
- # MAVLink vs ROS 2 Offboard Control
  collapsed:: true
	...
	- ## Mode Executors
		...
		- State machines that run through different modes, waiting for them to complete (similar to `navigator` module).
		...
		- #+BEGIN_IMPORTANT
		  User can always take over control from a mode or executor by commanding a mode switch via RC or GCS.
		  #+END_IMPORTANT
	- ## Control Interfaces
		- The mode:
			- decides what to control, how and when,
			- *streams* setpoints for controllers to track.
		- Mode space: What do I want to do? How do I do it? --> Control space: Control options?
